...
import { schedulingApi, type SchedulingFilterState, type SchedulingTechnicianEventPage } from "@/lib/scheduling-api";
...
export function TechnicianEventsDrillPanel({
  technicianId,
  title,
  filters,
  onClose,
      ...
      <Sheet open onOpenChange={(open) => !open && onClose()}>
        <SheetContent className="flex w-full flex-col sm:max-w-3xl">
          <SheetHeader className="border-b border-slate-100 bg-gradient-to-br from-slate-50 to-white">
            ...
            <SheetDescription>
              Reagendamentos gerados pessoalmente por este técnico no período
              {data ? ` · ${new Intl.NumberFormat("pt-BR").format(data.total)} reagendamento(s) no total` : ""}
            </SheetDescription>
          </SheetHeader>
                ...
                <Table>
                  ...
                  <TableBody>
                    ...
                    {data && !data.items.length ? (
                      <TableRow>
                        <TableCell colSpan={9} className="py-10 text-center text-sm text-slate-500">Nenhum reagendamento encontrado para este recorte.</TableCell>
                      </TableRow>
                    ) : null}
                  </TableBody>
                </Table>
