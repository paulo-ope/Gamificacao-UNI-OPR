"""operations backlog snapshot: add sector as a captured dimension
...
def upgrade() -> None:
    op.add_column(
        ...
        sa.Column("sector", sa.String(length=160), nullable=False, server_default="Não identificado"),
    )
...
def downgrade() -> None:
    op.drop_constraint(OLD_CONSTRAINT, "operations_backlog_snapshots", type_="unique")
    op.create_unique_constraint(OLD_CONSTRAINT, "operations_backlog_snapshots", ["snapshot_date", "regional", "team_model"])
    op.drop_column("operations_backlog_snapshots", "sector")
